...
function ClusterTree(coordinates; max_points_per_leaf::Int=32, dims::Int=1)
    ...
    root_node = build_tree_node(coordinates, max_points_per_leaf, point_indices, 1, size(coordinates, 2) + 1)
    tree = ClusterTree(point_indices, root_node)

    # If dims > 1, expand index_map and adjust tree node indices
    if dims > 1
        expanded_index_map = vec(vcat([(dims * point_indices .- (dims - i))' for i in 1:dims]...))
